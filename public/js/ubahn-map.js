...
const width = isMobile ? window.devicePixelRatio * window.screen.width : screen.width;
const height = isMobile ? window.devicePixelRatio * window.screen.height : screen.height;
...
function transliterateImageName(str) {
  ...
  return str
    ...
    .replace(new RegExp(`[${Object.keys(umlautMap).join('|')}]`, 'g'), m => umlautMap[m])
    .replace(/ /g, '_');
}
...
function handleImageColor() {
  if (Cookies.get('grayscale-photos') === 'true') {
    $('.wiki-image').css({ filter: 'grayscale(100%)', '-webkit-filter': 'grayscale(100%)' });
  }
}
...
function getWikiData(station, meta) {
  ...
  const imagePath = meta.image_cache
    ? concat('articles/images/', transliterateImageName(station.name), '.jpg')
    : null;
  ...
  const showContent = data => {
    ...
    showSidebar(concat(title, body, addendum(meta, 'en')));
    handleImageColor();
    $(".main-footer").hide();
  };
...
function showWikiData(station) {
  ...
  $('#sidebar-footer').html(
    concat(
      ...
      station.position.lat, '&mlon=', station.position.lon,
      '&zoom=16" target="_blank">', station.position.lat, ', ', station.position.lon, '</a>'
    ));
}
